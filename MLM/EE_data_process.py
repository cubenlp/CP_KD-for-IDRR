...
class EEProcessor(DataProcessor):
    ...
    def process_data_from_csv(self,data):
        ...
        for i,x in data.iterrows():
            ...
            # template: Sets the percentage of answering hints added， the best performance is when the percentage is 10%.

            # template 777
            if i%10==0:
                mask_text.append("{} <mask> {} Answer: {}".format(x["Arg1_RawText"],x["Arg2_RawText"],x["label"].split(".")[0].lower()))
            else:
    ...
    def create_dataloader(self,data,batch_size,shuffle=False,max_length=250):
        ...
        mask_text,label_conn = self.process_data_from_csv(data)
